...
你要为目标项目生成知识图谱初始骨架。图谱目录:`.claude/kg/`(应由 install 脚本预先创建)。

用户可选提供的系统列表:**$ARGUMENTS**
...
若 MCP 工具不可用 → 提示用户重启会话使 `.mcp.json` 生效,然后停止。
...
- 如果 `$ARGUMENTS` 给了系统列表,**以此为基础**(可能要补全到 10-20 个)
- 否则根据扫描结果推断
...
**注意:图谱 JSON 受 PreToolUse hook 保护,直接 Write/Edit 会被拦截。**

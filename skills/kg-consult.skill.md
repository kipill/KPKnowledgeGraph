...
### 情形5：能力目录已存在，但成员没录全（追加/补录）

目录成员多（几十~上百）时，常分批录入或先录骨架后补语义。遇到下面任一情况，建议追加：

- 用户明说"把 XxxType 剩下的成员也录进来" / "补全某目录"
- 读到某已录目录的源码，发现**图谱里的成员比源码少**（漏录了成员）
- 完善已有成员的 `scenarios`/`reuse_note`（先前写得潦草）

用 `kg_add_capability_members(catalog_id, members, reason)`——**merge 语义**：
enum_value 已存在→更新覆盖，新的→追加，其余不动。返回报告更新了几个/新增了几个。
整理好 + **用户确认后**再调（写入侧人在环里）。

> ⚠ **漏召回警觉**：`kg_scout_reuse` 返回"无命中"时，要分清是**图谱真没有**还是
> **对应成员没录进来**。若你读代码知道存在相关能力、但 scout 没召回，很可能是没录全——
> 提示用户补录（情形5），而不是当成"图谱没有"直接 fallback 到当新功能开发。
